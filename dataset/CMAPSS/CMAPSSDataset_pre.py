...
class CMAPSSDataset_pre(Dataset):
    ...
    def normalization(self):
        ...
        if self.norm_params is None:
            self.norm_params = CMAPSSDataset_pre.gen_norm_params([self], norm_type, norm_by_operations)
        norm_params = self.norm_params
        if norm_type == '0-1':
            min_norm, max_norm = 0, 1
            self.df = CMAPSSDataset_pre.min_max_normalization(df, norm_cols, norm_params, min_norm, max_norm)
        if norm_type == '-1-1':
            min_norm, max_norm = -1, 1
            self.df = CMAPSSDataset_pre.min_max_normalization(df, norm_cols, norm_params, min_norm, max_norm)
        if norm_type == 'z-score':
            self.df = CMAPSSDataset_pre.z_score_normalization(df, norm_cols, norm_params)
    ...
    @staticmethod
    def get_datasets(dataset_root, sub_dataset, mask_p, random_p, sequence_len=1, max_rul=None,
        ...
        train_df = pd.read_csv(os.path.join(dataset_root, 'train_{:s}.txt'.format(sub_dataset)), sep=' ',
                               header=None)
        train_df['label'] = [ 1 for x in range(train_df.shape[0])]
        test_df = pd.read_csv(os.path.join(dataset_root, 'test_{:s}.txt'.format(sub_dataset)), sep=' ', header=None)
        test_df['label'] = [1 for x in range(test_df.shape[0])]
        rul_df = pd.read_csv(os.path.join(dataset_root, 'RUL_{:s}.txt'.format(sub_dataset)), header=None)
        if sub_dataset == 'FD001':
            train_df2 = pd.read_csv(os.path.join(dataset_root, 'train_{:s}.txt'.format('FD003')), sep=' ',
                               header=None)
            train_df2.iloc[:, 0] = train_df2.iloc[:, 0] + train_df.iloc[-1, 0]
            train_df2['label'] = [0 for x in range(train_df2.shape[0])]
            test_df2 = pd.read_csv(os.path.join(dataset_root, 'test_{:s}.txt'.format('FD003')), sep=' ', header=None)
            test_df2.iloc[:, 0] = test_df2.iloc[:, 0] + test_df.iloc[-1, 0]
            test_df2['label'] = [1 for x in range(test_df2.shape[0])]
            rul_df2 = pd.read_csv(os.path.join(dataset_root, 'RUL_{:s}.txt'.format('FD003')), header=None)
        elif sub_dataset == 'FD003':
            train_df2 = pd.read_csv(os.path.join(dataset_root, 'train_{:s}.txt'.format('FD001')), sep=' ',
                               header=None)
            train_df2['label'] = [0 for x in range(train_df2.shape[0])]
            train_df2.iloc[:, 0] = train_df2.iloc[:, 0] + train_df.iloc[-1, 0]
            test_df2 = pd.read_csv(os.path.join(dataset_root, 'test_{:s}.txt'.format('FD001')), sep=' ', header=None)
            test_df2.iloc[:, 0] = test_df2.iloc[:, 0] + test_df.iloc[-1, 0]
            test_df2['label'] = [1 for x in range(test_df2.shape[0])]
            rul_df2 = pd.read_csv(os.path.join(dataset_root, 'RUL_{:s}.txt'.format('FD001')), header=None)
        elif sub_dataset == 'FD002':
            train_df2 = pd.read_csv(os.path.join(dataset_root, 'train_{:s}.txt'.format('FD004')), sep=' ',
                               header=None)
            train_df2['label'] = [0 for x in range(train_df2.shape[0])]
            train_df2.iloc[:, 0] = train_df2.iloc[:, 0] + train_df.iloc[-1, 0]
            test_df2 = pd.read_csv(os.path.join(dataset_root, 'test_{:s}.txt'.format('FD004')), sep=' ', header=None)
            test_df2.iloc[:, 0] = test_df2.iloc[:, 0] + test_df.iloc[-1, 0]
            test_df2['label'] = [1 for x in range(test_df2.shape[0])]
            rul_df2 = pd.read_csv(os.path.join(dataset_root, 'RUL_{:s}.txt'.format('FD004')), header=None)
        elif sub_dataset == 'FD004':
            train_df2 = pd.read_csv(os.path.join(dataset_root, 'train_{:s}.txt'.format('FD002')), sep=' ',
                               header=None)
            train_df2.iloc[:, 0] = train_df2.iloc[:, 0] + train_df.iloc[-1, 0]
            train_df2['label'] = [0 for x in range(train_df2.shape[0])]
            test_df2 = pd.read_csv(os.path.join(dataset_root, 'test_{:s}.txt'.format('FD002')), sep=' ', header=None)
            test_df2.iloc[:, 0] = test_df2.iloc[:, 0] + test_df.iloc[-1, 0]
            test_df2['label'] = [1 for x in range(test_df2.shape[0])]
            rul_df2 = pd.read_csv(os.path.join(dataset_root, 'RUL_{:s}.txt'.format('FD002')), header=None)
        train_df = pd.concat([train_df,train_df2], ignore_index=True)
        test_df = pd.concat([test_df, test_df2], ignore_index=True)
        rul_df = pd.concat([rul_df, rul_df2], ignore_index=True)
        ...
        # print
        train_dataset = CMAPSSDataset_pre(
            train_df,
            init=False,
            **dataset_kwargs
        )
        ...
        test_dataset = CMAPSSDataset_pre(
            test_df,
            init=False,
            **dataset_kwargs
        )
        ###############
        # 调整test_dataset.df的id，为训练集和测试机拼接做准备
        #         start_id = train_dataset.df['id'].unique().max() + 1

        #         for test_id in test_dataset.df['id'].unique():
        #             idx_list = test_dataset.df[test_dataset.df.id == test_id].index.tolist()
        #             test_dataset.df.loc[idx_list,'id'] = start_id
        #             start_id = start_id + 1

        # train_dataset.df = pd.concat([train_dataset.df, test_dataset.df])
        # print(train_dataset.df.shape) 33727X21
        ##################
        ...
        if cluster_operations:
            CMAPSSDataset_pre.clustering_operations(dataset_list)  # FD002和FD004进行聚类操作，FD001和FD003不聚类   ？

        if norm_type:
            norm_params = CMAPSSDataset_pre.gen_norm_params(dataset_list, norm_type, norm_by_operations)
            for dataset in dataset_list:
                dataset._set_norm_params(norm_type, norm_params, norm_by_operations)
                dataset.normalization()

        # train_df = pd.DataFrame(train_dataset.df)
        # train_df.to_csv('./FD004.csv',index = False)

        for dataset in dataset_list:
            dataset.gen_pre_sequence()
            # print(dataset.pretrain_sequence_array.shape)   #7931X128X17
            # print(dataset.labels.shape)                  #7931,

        # train_df = pd.DataFrame(train_dataset.pretrain_sequence_array[0])
        # train_df.to_csv('./1.csv',index = False)
        # test_df = pd.DataFrame(test_dataset.pretrain_sequence_array[0])
        # test_df.to_csv('./2.csv',index = False)

        # print(train_dataset.pretrain_sequence_array.shape) #24966X46X17
        # print(train_dataset.labels.shape) #24966,
    ...
    def __init__(self, data_df, mask_p, random_p, sequence_len=1, final_rul=None, norm_params=None, norm_type=None,
        ...
        # set self.df
        self.df = data_df
        self.df = data_df.iloc[:, [0, 1, 2, 3, 4, 6, 7, 8, 11, 12, 13, 15, 16, 17, 18, 19, 21, 24, 25, 28]]
        self.df.columns = CMAPSSDataset_pre.DATASET_COLS
        ...
        # feature cols define
        self.feature_cols = CMAPSSDataset_pre.OPERATION_COLS + CMAPSSDataset_pre.SENSOR_COLS
        ...
        # pretrain data
        self.pretrain_sequence_array = None
        self.labels = None
        self.label2 = None
    ...
    def __len__(self):
        return len(self.pretrain_sequence_array)
    ...
    def __getitem__(self, i):
        ...
        '''
        l = [torch.FloatTensor(self.pretrain_sequence_array[i]), torch.FloatTensor([self.labels[i]]), torch.LongTensor([self.label2[i]])]
    ...
    def gen_pre_sequence(self):
        SEED = 529
        random.seed(SEED)

        seq_cols = ['id'] + self.feature_cols + ['rul', 'label']
        ...
        for id in self.df['id'].unique():
            ...
            if row_num >= seq_len:
                if self.only_final:
                    labels.append(id_array[-1, -2])
                    label2.append(id_array[-1, -1])
                    all_array.append(id_array[row_num - seq_len:])
                else:
                    for i in range(0, row_num - seq_len + 1):
                        data_array = id_array[i: i + seq_len].copy()
                        rand_num = random.random()
                        if rand_num < self.mask_p:
                            data_array[-1, 1:-2] = [0]
                        elif self.mask_p < rand_num < self.mask_p + self.random_p:
                            data_array[-1, 1:-2] = [random.random() for i in range(data_array.shape[1] - 3)]
                            data_array[-1, 2] = 0

                        labels.append(data_array[-1, -2])
                        label2.append(data_array[-1, -1])
                        all_array.append(data_array)

            else:
                # row number < sequence length, only one sequence
                # pad with first time-cycle value
                if self.only_final:
                    labels.append(id_array[-1, -2])
                    label2.append(id_array[-1, -1])
                    all_array.append(np.pad(id_array, ((seq_len - id_array.shape[0], 0), (0, 0)), 'edge'))
                else:
                    data_array = id_array.copy()
                    rand_num = random.random()
                    if rand_num < self.mask_p:
                        data_array[-1, 1:-2] = [0]
                    elif self.mask_p < rand_num < self.mask_p + self.random_p:
                    ...
                    labels.append(data_array[-1, -2])
                    label2.append(data_array[-1, -1])
                    all_array.append(np.pad(data_array, ((seq_len - data_array.shape[0], 0), (0, 0)), 'edge'))

        all_array = np.stack(all_array)
        self.pretrain_sequence_array = all_array[:, :, 1:-2]
        self.labels = np.array(labels)
        self.label2 = np.array(label2)
